...
async function handleVerify() {
  ...
  try {
    ...
    // 加入缓存
    authStore.setToken(userName+pwd)
    // 成功弹框
    ms.success('success')
    // 重载页面
      ...
      <div class="space-y-4">
        <header class="space-y-2">
          ...
          <Icon403 class="w-[200px] m-auto" />
				...
				<text>用户名：</text><NInput v-model:value="username" type="username" placeholder="" @keypress="handlePress" />
				<text>密码：</text><NInput v-model:value="password" type="password" placeholder="" @keypress="handlePress" />
        <NButton
          block
          type="primary"
          :disabled="disabled"
